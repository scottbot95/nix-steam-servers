...
let
  ...
  writeXML = value:
    let
      properies = mapAttrsToList 
        (name: propVal:
          let 
            encoded = if (builtins.typeOf propVal) == "bool" then boolToString propVal else toString propVal;
  ...
  config =
    ...
    mkIf ((builtins.length (builtins.attrNames enabledServers)) > 0) {
      ...
      systemd.services = mapAttrs'
        (name: conf:
          let
            configFile = writeXML conf.config;
            args = [
              "-logfile"
              conf.logFile
              "-quit"
          ...
          in
          {
            ...
            value = rec {
              ...
              script = ''
                ${conf.package}/7DaysToDieServer.x86_64 ${concatStringsSep " \\\n" args}
              '';
